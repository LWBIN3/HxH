...
function getBondTypes(atoms) {
  ...
  points.forEach((point) => {
    const neighbors = tree
      .nearest(point, 2, Infinity)
      .filter(([neighborPoint, dist]) => dist > 0.01);

    if (neighbors.length > 0) {
      const [neighborPoint, minDist] = neighbors[0];
      minimumDistances.set(point.join(","), minDist);
    }
  });
  ...
  points.forEach((point) => {
    ...
    const minDist = minimumDistances.get(point.join(","));

    if (!minDist) return;

    const bondThreshold = minDist * scaleFactor;
    const potentialBonds = tree
      .nearest(point, 10, bondThreshold)
      .filter(([neighborPoint, dist]) => dist > 0.01);
